...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'T', Name = 'A-SAB Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"A-SAB Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"A-SAB Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "UI Hidden", Content = "The UI has been hidden, you can unhide the UI by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			RayfieldLibrary:Notify({Title = "UI Hidden", Content = `The UI has been hidden, you can unhide the UI by tapping {getSetting("General", "rayfieldOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.ShowText then
		MPrompt.Title.Text = 'Unhide '..Settings.ShowText
	end

	LoadingFrame.Version.TextTransparency = 1
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Loading The Script.."
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Please Wait!"

	if Settings.LoadingTitle ~= "Loading The Script.." then
		LoadingFrame.Version.Text = "V1.0.1"
	end
		...
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					Title = "A-SAB Notification",
					Content = "Enjoying this script? Speard it for updates!",
					Duration = 7,
					Image = 4370033185,
				})
			end
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "rayfield")]) and not processed then
		if Debounce then return end
		if  then
			Hidden = false
			Unhide()
